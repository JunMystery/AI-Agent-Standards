...
def load_agent_choices(standards_dir: str) -> list[dict]:
    """Load installable agent instruction files from the generator manifest."""
    manifest_path = os.path.join(standards_dir, "rules", "agent-manifest.json")
...
def all_choice_files(agent_choices: list[dict]) -> list[str]:
    return [
        file_path
...
def find_project_root(start_dir: str) -> str:
    ...
    markers = (
        ".git", ".hg", "package.json", "Cargo.toml", "go.mod", "pom.xml",
...
def copy_and_link(standards_dir: str, project_root: str, rule_files: list[str]) -> None:
    ...
    for rel_path in rule_files:
        ...
        if rel_path == PROJECT_STANDARDS_FILE and os.path.exists(dst):
...
def update_gitignore(project_root: str, standards_dir: str = None, installed_files: list[str] = None) -> None:
    ...
    for line in lines:
        ...
        if stripped and not stripped.startswith("#"):
            existing_rules.add(stripped.lstrip("/").replace("\\", "/"))

    to_add = [item for item in files_to_ignore if item not in existing_rules]
...
def main():
    ...
    agent_choices = load_agent_choices(standards_dir)
